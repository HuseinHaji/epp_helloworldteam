...
The following graph shows the contents of assignment_4 root directory after
executing `pytask`
...
The contents of the root/src/assignment_4 directory follow the steps of the analysis
...
- **`public/`**: Directory containing figure files (PNG format) that are generated by
  tasks in `src/assignment_4/final/task_final_template.py`. Figures are placed here
...
- **`tables/`**: Directory containing table files (Markdown format) that are generated
  by tasks in `src/assignment_4/final/task_final_template.py`. Tables are written in
  Markdown format for inclusion in MyST documents.

The tasks in `src/assignment_4/final/task_final_template.py` generate:
